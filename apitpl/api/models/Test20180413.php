    ...
    public function account_list($province_id,$city_id,$area_id,$level)
    {
        ...
        if ($data === false) { 
            ...
            $result = $query->from('province_city_area')->select('name,id,sort')->all();
             ...
             $sql = "select a.`id`,a.`province_id`,a.`city_id`,a.`area_id`,a.`title`,a.`teacher_count`,a.`school_type`,a.`class_info`,a.`sort`,p.`sort` as psort  from account as a,province_city_area as p where a.`area_id`= p.`id` ".$w;
    ...
    public function account_class($province_id,$city_id,$area_id,$school_id,$level)
    {
        ...
        if ($data === false) { 
            ...
            foreach ($data as $k => $v) {
                ...
                $h = 0; //去重
                $sql = 'select g.`title`,c.`extend` as year,g.`extend`,g.`id`,c.`title` as grade from contacts_group as c,contacts_group as g where c.`id`=g.`pid` and g.`qid`='.$v['id'].' and g.`type`=6 and g.`status`=1 order by c.`extend`&4095 desc,g.`extend` asc';
